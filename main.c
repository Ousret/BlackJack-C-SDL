	...
	while (1) {
		...
		switch (UserChoise) {
			...
			case 0: 
				...
				if (SDL_generate(popup) == btnOK) {
					...
					if (BJ_setBet(1, atoi(mise))) {
						...
						SDL_delObj(popup, 2);
						SDL_delObj(popup, 1);
						SDL_modObj(popup, 0, 0, "OK", ALIGN_CENTER, NULL, NONE, 150, 200);
						SDL_delText(popup, 3);
						SDL_delText(popup, 2);
						SDL_delText(popup, 1);
			...
			case 1:
				...
				SDL_newSprite(popup, "panda_sp.png", colorGreenLight, 35, 35, 10, 100, 4, 4, 0); //Just for testing sprite
...
void handleBlackJack() {
	...
	SDL_loadSound("Female/killingspree.wav");
	...
	if (BJ_getScore(1) == 21) {
		...
		SDL_playSound("Female/killingspree.wav");
	...
	SDL_newObj(ingame, NULL, 0, "Abandon", ALIGN_CENTER, NULL , NONE, 50, 530);
	SDL_newObj(ingame, NULL, 0, "Rester", ALIGN_CENTER, NULL, NONE, 280, 530);
	SDL_newObj(ingame, NULL, 0, "Frapper", ALIGN_CENTER, NULL, NONE, 510, 530);
...
void handleHUD(t_context * context, char * scoreDealer, char * scorePlayer, char * betPlayer, char * soldePlayer) {
	...
	if (context->nbText == 0) {
		...
		if (BJ_getScore(0) > 21) {
			...
			SDL_modText(context, 0, scoreDealer, colorRed, 330, 100);
			
		}else{
		
			SDL_modText(context, 0, scoreDealer, colorWhite, 330, 100);
			
		}
		
		if (BJ_getScore(1) > 21) {
		
			SDL_modText(context, 1, scorePlayer, colorRed, 330, 380);
			
		}else{
		
			SDL_modText(context, 1, scorePlayer, colorWhite, 330, 380);
			
		}
		
		SDL_modText(context, 2, betPlayer, colorWhite, 350, 360);
		SDL_modText(context, 3, soldePlayer, colorWhite, 115, 580);
